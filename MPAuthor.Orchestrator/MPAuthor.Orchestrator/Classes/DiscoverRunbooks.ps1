#param($sourceId, $managedEntityId, $computerName, $serviceUrl, $debug)
param($sourceId, $managedEntityId, $computerName, $MgmtServerInstallPath, $debug)

$SCRIPT_NAME = 'DiscoverRunbooks.ps1'
...
$url = Get-OrchestratorServiceUrl -Server $webServer -port $port -query 'Runbooks()' 
#$url = $serviceUrl.Trim() + '/Runbooks()'
...
Log-DebugEvent -eventNo 221 -message $message
...
if (($rbarray.count -eq 0) -or ($rbarray -eq $null))
{
	...
	$api.LogScriptEvent($SCRIPT_NAME,225,$EVENT_LEVEL_ERROR,$message)
...
else
{
	foreach ($rb in $rbarray)
	{
		...
		$rbPathName = $rb.Path
		$rbId = $rb.Id
		$rbIsMonitor = $rb.IsMonitor
		$rbWebPath = $rb.Url

		$message =  "Computer: " + $computerName + "`n" + `
					"Name: " + $rbName + "`n" + `
					"Path: " + $rbPathName + "`n" + `
					"ID: " + $rbId + "`n" + `
					"WebPath: " + $rbWebPath + "`n" + `
					"IsMonitor: " + $rbIsMonitor 
		Log-DebugEvent -eventNo 222 -message $message
    
		$instance = $discoveryData.CreateClassInstance("$MPElement[Name='MPAuthor.Orchestrator.Runbook']$")
		$instance.AddProperty("$MPElement[Name='Windows!Microsoft.Windows.Computer']/PrincipalName$", $computerName)
		$instance.AddProperty("$MPElement[Name='Orch2012!Microsoft.SystemCenter.2012.Orchestrator.Role.ManagementServer']/InstallPath$", $MgmtServerInstallPath)
		$instance.AddProperty("$MPElement[Name='MPAuthor.Orchestrator.Runbook']/ID$", $rbId)
		$instance.AddProperty("$MPElement[Name='MPAuthor.Orchestrator.Runbook']/Name$", $rbName)
		$instance.AddProperty("$MPElement[Name='MPAuthor.Orchestrator.Runbook']/Path$", $rbPathName)
		$instance.AddProperty("$MPElement[Name='MPAuthor.Orchestrator.Runbook']/WebPath$", $rbWebPath)
		$instance.AddProperty("$MPElement[Name='MPAuthor.Orchestrator.Runbook']/IsMonitor$", $rbIsMonitor)
		$instance.AddProperty("$MPElement[Name='System!System.Entity']/DisplayName$", $rbName)
